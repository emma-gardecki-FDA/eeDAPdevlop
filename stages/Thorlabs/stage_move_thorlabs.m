...
%% ###################### MOVE STAGE SEQUENCE ###############################
%  ##########################################################################
function stage=stage_move_thorlabs(stage, target_pos, h_stage)
%--------------------------------------------------------------------------
...
try
    ...
    if exist('h_stage', 'var') == 0
        h_stage_close = 1;
        [stage] = stage_move_thorlabs(stage.label); %stage_open_pior
    ...
    stage_send_com_thorlabs (stage.handle, command_str_speed);
    command_str_accel = ['ACCEL',...
        ' x=', num2str(stage.accel),...
        ' y=',num2str(stage.accel)];
    stage_send_com_thorlabs (stage.handle, command_str_accel);           
    stage_send_com_thorlabs (stage.handle, 'SPEED X Y');   
    stage_send_com_thorlabs (stage.handle, 'ACCEL X Y');
    command_str = ['move',...
        ' x=', num2str(target_pos(1)),...
        ' y=',num2str(target_pos(2))];
    stage_send_com_thorlabs (stage.handle, command_str);
